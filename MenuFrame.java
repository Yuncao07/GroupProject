...
/**
 * This class sets up all required panels and storages in which users can move
 * between panels and guest users may book reservations which will be stored,
 * and managers can access storage and view them in two different ways.
 * 
 * @author
 *
 */
public class MenuFrame extends JFrame {
	...
	public MenuFrame() {
		...
		guestButton.addActionListener(event -> { // moves user to guest section
													// of program
			login.manager(false);
		...
		managerButton.addActionListener(event -> { // moves user to manager
													// section of program
			login.manager(true);
			getContentPane().removeAll();
		...
		loginButton.addActionListener(event -> {
			...
			if (login.isManager()) {
				getContentPane().removeAll();
				getContentPane().add(managerOptions); // go to jpanel with
														// manager options
			} else {
		...
		signUp.addActionListener(event -> { // directs user to create a new
											// account
			getContentPane().removeAll();
		...
		make.addActionListener(event -> { // make a reservation
			...
			viewOrCancel.addActionListener(event -> {
				viewCancel = new ReservationViewCancel(current.getReservationList());
				viewCancel.setLayout(new BorderLayout());						
				getContentPane().removeAll();
		...
		datePicking.addListenerToSEButton(event -> { // user selected economic
														// room
			RoomList.roomList.luxRoom(false);
		});

		datePicking.addListenerToSLButton(event -> { // user selected luxurious
														// room
			RoomList.roomList.luxRoom(true);
		});

		datePicking.addListenerToSButton(event -> { // show available rooms
			if (DateInput.parseDate(datePicking.getStart()) != null
					&& DateInput.parseDate(datePicking.getEnd()) != null) {
				if (DateReservation.allowedDates(DateInput.parseDate(datePicking.getStart()),
						DateInput.parseDate(datePicking.getEnd()))) {
					RoomList.roomList.setDateReservation(new DateReservation(
							DateInput.parseDate(datePicking.getStart()), DateInput.parseDate(datePicking.getEnd())));
					getContentPane().removeAll();
					getContentPane().add(reservePanel); // add reservation date
														// here
					revalidate();
					repaint();
					pack();
		...
		JButton confirm = new JButton("Confirm"), more = new JButton("More Reservations"), done = new JButton("Done"),
				econ = new JButton("$100"), lux = new JButton("$300"), change = new JButton("Confirm Change of Date");

		confirm.addActionListener(event -> { // add to list of reservations made
												// by person
		...
		change.addActionListener(event -> {
			...
			if (DateInput.parseDate(sDate.getText()) != null && DateInput.parseDate(eDate.getText()) != null) {
				if (DateReservation.allowedDates(DateInput.parseDate(sDate.getText()),
						DateInput.parseDate(eDate.getText()))) {
					RoomList.roomList.setDateReservation(new DateReservation(DateInput.parseDate(sDate.getText()),
							DateInput.parseDate(eDate.getText())));
				} else {
		...
		simple.addActionListener(event -> {
			/*
			 * selectedReceipt = new SimpleReceipt(current); displayReceipt =
			 * new ReceiptPanel(selectedReceipt); getContentPane().removeAll();
			 * getContentPane().add(displayReceipt); revalidate(); repaint();
			 */
		...
		displayReceipt.addListener(event -> { // make the button in receipt
												// panel to return to main menu
			current = null;
		...
		JButton load = new JButton("Load"), view = new JButton("View"), save = new JButton("Save"),
